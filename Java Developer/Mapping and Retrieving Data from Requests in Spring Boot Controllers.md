...
Мэппинг позволяет разработчикам настроить маршрутизацию запросов (определить, какой метод контроллера будет вызван в ответ на определённый запрос) и указать, какие типы запросов (`GET`, `POST`, `PUT`, `PATCH`, `DELETE` и т.д.) и URL-адреса (пути) связаны с этими методами.

Методы HTTP `GET`, `POST`, `PUT`, `PATCH` и `DELETE` используются для выполнения различных операций с ресурсами на сервере:
...
Данная диаграмма иллюстрирует базовый процесс взаимодействия с REST (Representational State Transfer) API, в которой REST контроллер выступает в роли посредника, обрабатывая входящие запросы к соответствующим эндпоинтам и возвращая соответствующие ответы:

`1)` Запрос (request): Процесс начинается с того, что клиент (изображен в виде компьютера слева) отправляет HTTP-запрос на определенный эндпоинт. В данном случае запрос отправляется на "http://localhost:8080/hello".

`2)` REST контроллер (Rest controller): Запрос принимается REST контроллером (изображен синей иконкой справа). REST контроллер - это компонент в веб-приложениях, который обрабатывает входящие HTTP-запросы и определяет, как на них отвечать.

`3)` Эндпоинты (endpoints): Внутри REST контроллера находятся различные эндпоинты (изображены в виде древовидной структуры). Каждый эндпоинт соответствует определенной комбинации URL-пути и HTTP-метода, определяя различные функциональности или ресурсы API.

`4)` Ответ (response): После обработки запроса REST контроллер отправляет HTTP-ответ обратно клиенту.
